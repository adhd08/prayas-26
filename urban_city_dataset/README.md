...
Please retain these credits when sharing, publishing, or redistributing outputs from this project:

- **GHSL / GHS-UCDB:** European Commission, Joint Research Centre (JRC), Global Human Settlement Layer. Cite: Rivero et al. (2024), *GHS-UCDB R2024A - GHS Urban Centre Database 2025*, DOI [10.2905/1a338be6-7eaf-480c-9664-3a8ade88cbcd](https://doi.org/10.2905/1a338be6-7eaf-480c-9664-3a8ade88cbcd). Reuse authorised under the European Commission reuse policy, with source acknowledgement.
- **GHSL population grid:** European Commission, Joint Research Centre, *GHS-POP E2025 GLOBE R2023A*. Cite the [official GHSL download](https://jeodpp.jrc.ec.europa.eu/ftp/jrc-opendata/GHSL/GHS_POP_GLOBE_R2023A/GHS_POP_E2025_GLOBE_R2023A_54009_1000/V1-0/).
- **Overture Maps:** Overture Maps Foundation, release `2026-08-19.0`. Overture data includes contributions from multiple providers; retain each feature's `sources_json` and follow the [Overture attribution and licensing requirements](https://docs.overturemaps.org/attribution/). Do not describe all Overture features as having one common licence.
- **OpenStreetMap:** © OpenStreetMap contributors. Data available under the [Open Database License (ODbL) 1.0](https://opendatacommons.org/licenses/odbl/). OSM was queried through Overpass and used as a supplementary source for the pilot cities.
- **Eurostat:** European Commission, Eurostat, City Statistics / Urban Audit. Cite the [Eurostat City Statistics metadata](https://ec.europa.eu/eurostat/cache/metadata/en/urb_esms.htm). Eurostat observations remain separate administrative-city context data.
- **Copernicus DEM:** ESA/Copernicus Programme, *Copernicus DEM GLO-30*, distributed through the public AWS registry. Follow the [Copernicus DEM licence](https://dataspace.copernicus.eu/explore-data/data-collections/copernicus-contributing-missions/collections-description/COP-DEM).
- **Upstream imagery and indicators:** GHSL indicator definitions retain upstream acknowledgements in `data/raw/ghsl/GHSL_UCDB_R2024_V1_2.txt` and `data/processed/city_level/ghsl_indicator_dictionary.csv`, including Copernicus Land Monitoring Service, Sentinel/Landsat, USGS, UN-Habitat and other cited contributors where applicable.

Suggested acknowledgement: “This work uses data from the European Commission Joint Research Centre GHSL, Overture Maps Foundation, OpenStreetMap contributors, Eurostat, and the ESA/Copernicus Programme. Licences and attribution requirements for each source are listed above and in `city_sources.csv`.” Data access and cache metadata were recorded on 19 September 2026; source releases and URLs are pinned in `config.json` and per-field in `city_sources.csv`.

...
def test_crossing_road_is_clipped_not_discarded():
    g = gpd.GeoDataFrame(
        {
            "id": ["a", "b"],
            "geometry": [LineString([(-1, 0.5), (2, 0.5)]), Point(3, 3)],
        },
        crs=4326,
    )
    result, stats = clip(g, box(0, 0, 1, 1))
    assert len(result) == 1
    assert np.allclose(result.total_bounds, [0, 0.5, 1, 0.5])
...
def test_duplicate_names_only_merge_within_distance_and_category():
    g = gpd.GeoDataFrame(
        {
            "poi_id": ["a", "b", "c", "d", "e"],
            "poi_name": ["Hospital", "Hospital", "Hospital", "Hospital", None],
            "poi_category": ["hospital", "hospital", "hospital", "clinic", "hospital"],
            "geometry": [
                Point(0, 0),
                Point(20, 0),
                Point(100, 0),
...
def test_building_union_avoids_overlap_double_counting():
    g = gpd.GeoDataFrame(
        {"id": ["a", "b"], "geometry": [box(0, 0, 100, 100), box(50, 0, 150, 100)]},
        crs=3857,
    )
    f = building_features(g, 3857, 1)
    assert f["building_footprint_area_km2"] == 0.02
    assert f["building_footprint_union_area_km2"] == 0.015

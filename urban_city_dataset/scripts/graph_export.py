...
not just the nonzero-population subset. Each cell carries:
  ...
  building_density_pct - % of the 1km cell covered by the union of building footprints
...
from grid_features import building_density_pct
...
GREEN_TYPE_MIN_PCT = (
    40.0  # green land use must actually cover the cell before it becomes the cell Type
)
DEM_TEMPLATE = (
...
def build_grid(city_id, boundary):
    ...
    with rasterio.open(POP_FILE) as src:
        ...
        gdf = gpd.GeoDataFrame(
            {"population": pop, "row": rr, "col": cc}, geometry=cells, crs=src.crs
        )
    gdf = gdf.to_crs(4326)
...
def dominant_type(polygons, class_col, class_map, subtype_col, subtype_map, cells, crs):
    ...
    p = polygons.copy()
    mapped = (
        p[class_col].map(class_map)
        if class_col in p
        else pd.Series(index=p.index, dtype=object)
    )
    if subtype_col in p:
...
def export_cities():
    ...
    cities.rename(columns={"city_id": "city_id:ID(City)"}).to_csv(
        GRAPH_OUT / "cities.csv", index=False
    )
...
def grid_neighbors(cells):
    ...
    lookup = {
        (r, c): cid for r, c, cid in zip(cells["row"], cells["col"], cells["cell_id"])
    }
    edges = []
...
def city_cells(city_id, boundary):
    ...
    landuse_type = dominant_type(
        land_use, "class", LANDUSE_CLASS_TYPE, None, {}, cells, crs
    )
    built_land_use = (
        land_use[~land_use["class"].isin(GREEN_CLASSES)]
        if land_use is not None
        else None
    )
    landuse_built_type = dominant_type(
        built_land_use, "class", LANDUSE_CLASS_TYPE, None, {}, cells, crs
    )
    green_pct = green_cover_pct(land_use, cells, crs)
    building_pct = building_density_pct(buildings, cells, crs)
    dist_bound = dist_to_boundary_m(cells, boundary, crs)
    elevation = sample_elevation(
        cells["centroid_lon"].to_numpy(), cells["centroid_lat"].to_numpy()
    )
    ...
    for cid, pop in zip(cells["cell_id"], cells["population"]):
        ...
        if cid in building_type:
            cell_type.append(building_type[cid])
            type_source.append("buildings")
        elif lu is not None and lu != "Green":
            cell_type.append(lu)
            type_source.append("land_use")
        elif lu == "Green" and green_pct.get(cid, 0.0) >= GREEN_TYPE_MIN_PCT:
            cell_type.append("Green")
            type_source.append("land_use")
            ...
            cell_type.append(landuse_built_type[cid])
            type_source.append("land_use")
        else:
            cell_type.append("Unbuilt" if pop == 0 else "Residential")
            type_source.append("heuristic")

    cells["type"] = cell_type
    cells["type_source"] = type_source
    cells["green_cover_pct"] = [
        round(float(green_pct.get(cid, 0.0)), 1) for cid in cells["cell_id"]
    ]
    if building_pct is None:
        cells["building_density_pct"] = np.nan
        cells["building_density_source"] = "missing_buildings"
    else:
        cells["building_density_pct"] = [
            building_pct.get(cid, 0.0) for cid in cells["cell_id"]
        ]
        cells["building_density_source"] = "overture_buildings"
...
def export_grid_cells(city_centroids):
    ...
    out = grid_df.rename(
        columns={
            ...
            "building_density_pct": "building_density_pct:float",
    ...
    pd.DataFrame(
        contains_rows, columns=[":START_ID(City)", ":END_ID(GridCell)"]
    ).to_csv(GRAPH_OUT / "city_contains.csv", index=False)
    pd.DataFrame(
        connected_rows,
        columns=[":START_ID(GridCell)", ":END_ID(GridCell)", "distance_m:float"],
    ).to_csv(GRAPH_OUT / "grid_connected.csv", index=False)

    print(
        f"grid_cells: {len(out)}  city_contains: {len(contains_rows)}  grid_connected: {len(connected_rows)}"
    )
...
def export_pois(city_centroids, grid_geo):
    ...
    for city_id in city_centroids["city_id"]:
        ...
        joined = gpd.sjoin(
            poi_pts, cells[["cell_id", "geometry"]], predicate="within", how="left"
        )
    ...
    located_df = pd.DataFrame(
        located_out, columns=[":START_ID(POI)", ":END_ID(GridCell)"]
    )
